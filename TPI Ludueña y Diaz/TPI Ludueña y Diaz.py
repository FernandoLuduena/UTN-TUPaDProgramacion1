...
def agregar_pais():
    ...
    with open(ARCHIVO, mode="a", newline="") as f:
        escritor = csv.writer(f)
        escritor.writerow([nombre,int(poblacion),int(superficie), continente])
        
    print(f"Pais {nombre} agregado exitosamente.")
    
def actualizar_pais():
    print("-----Actualizar Pais-----")
    nombre_buscar = input("Ingrese el pais que desea actualizar: ").strip()
    
    if not nombre_buscar:
        print("Error: El nombre del pais es obligatorio.")
        return
...
def buscar_pais():
    ...
    print("-----Buscar Pais-----")
    termino = input("Ingrese una parte o todo el nombre del pais: ")
